...
* fMRIPrep
   ...
   * Production Testing:
      ...
      ** plotted Euler numbers generated by freesurfer_audit and plotted distribution. Sub-15333 excluded from subsequent analyses (Euler=782). Reviewed sub-11305 (238) and sub-11399 (224) with Ted but ok'd to process
* XCP
    * tried to write timing files (task event array convolved with HRF) from BIDS events.tsv's for each run using [xcp_task_array.ipynb]() but stick files don't include ITI or decision phases, don't think this will work with xcp timing array which doesn't include onset times (i.e. need all TRs to be listed in rows)
       * ONSET TIMES IN STICK FILES REFLECT FACT THAT ANALYSIS PIPELINE DELETED FIRST 20 SECONDS=10TR OF BOLD RUNS, DURING WHICH TWO "DUMMY" TASK TRIALS OCCURRED
